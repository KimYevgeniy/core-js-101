...
function isInsideCircle(circle, point) {
  const point1 = (point.x - circle.center.x) ** 2;
  const point2 = (point.y - circle.center.y) ** 2;
...
function getDigitalRoot(num) {
  ...
  let sum = arr.reduce((acc, el) => acc + parseInt(el, 10), 0);
  if (sum > 9) {
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (brackets[str[i]]) {
      ...
      if (brackets[lastOpenedBracket] !== str[i]) {
        return false;
      }
    }
  }
...
function getCommonDirectoryPath(pathes) {
  let count = -1;
  let temp = '';
  for (let i = 0; i < pathes[0].length - 1; i += 1) {
    for (let j = 0; j < pathes[0].length - 1; j += 1) {
      if (pathes[i][j] !== pathes[i + 1][j]) {
        return temp.length > 1 ? temp.slice(0, -count) : temp;
      }
      if (pathes[i][j] === '/') {
        count = -1;
      }
